...
import { str, num, MONO, useColors, Chip, toolIcon, parseTodoLine, parseAsk, styles, type TodoItem } from "../kit";
...
function Task({ subagent, desc }: { subagent?: string; desc?: string }) {
  return <Chip icon="✦" label={`subagent · ${str(subagent) || "agent"}`} detail={str(desc)} />;
}
function Web({ query, url }: { query?: string; url?: string }) {
  return <Chip icon="🌐" label={query ? "web search" : "fetch"} detail={str(query) || str(url)} />;
}
function Skill({ name, args }: { name?: string; args?: string }) {
  return <Chip icon="✦" label={`/${str(name) || "skill"}`} detail={str(args)} accentLabel mono />;
}
...
function Todo({ body }: { body?: string }) {
  ...
  return (
    <View style={[styles.block, { borderColor: c.border, backgroundColor: c.card }]}>
      <Text style={{ fontSize: 12, fontWeight: "600", color: c.muted, marginBottom: 2 }}>☑  Plan</Text>
      {items.map((it, i) => (
...
function Collapsible({ icon, summary, body, mono, muted }: { icon: string; summary: string; body?: string; mono?: boolean; muted?: boolean }) {
  ...
  if (!text) return null;
  return (
    <View style={[styles.block, { borderColor: c.border, backgroundColor: c.card, gap: 0 }]}>
      <Pressable onPress={() => setOpen((o) => !o)} style={{ flexDirection: "row", gap: 7, alignItems: "center" }}>
        <Text style={{ color: c.subtle, fontSize: 11 }}>{open ? "▾" : "▸"}</Text>
        <Text style={{ fontSize: 12, fontWeight: "600", color: c.muted }}>{icon} {summary}</Text>
      </Pressable>
      {open ? (
...
function Bash({ body }: { body?: string }) { return <Collapsible icon="❯" summary="Output" body={body} mono />; }
function Thinking({ body }: { body?: string }) { return <Collapsible icon="💭" summary="Thought process" body={body} muted />; }
    ...
    <View style={[styles.block, { borderColor: c.border, backgroundColor: c.card, gap: 12 }]}>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
        {tiles.map(([label, value]) => (
          <View key={label} style={{ minWidth: 80, flexGrow: 1, padding: 10, borderRadius: 9, borderWidth: 0.5, borderColor: c.border }}>
