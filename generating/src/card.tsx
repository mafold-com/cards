...
/** A rotating ring — the "Stopping…" spinner. Its own component so the 700ms
 *  rotation ticks re-render only this tiny View, never the whole card. */
function Spinner({ color, size = 11 }: { color: string; size?: number }) {
...
function Generating({ started, beat, tokens, shells }: GeneratingProps) {
  ...
  // Stop is NOT instant — the tap relays server-side and the stream keeps
  // trickling until the daemon actually cancels. Latch a local "stopping"
  // state on tap so the button immediately shows a spinner + "Stopping…"
  // instead of looking dead (the reported "点不动" feeling). Survives the
  // ~300ms snapshot re-renders (same instance, like the Sparkle) and clears
  // when the finalized reply drops this card.
  ...
  return (
    ...
    >
      ...
      <Pressable
        disabled={stopping}
        onPress={() => { if (stopping) return; setStopping(true); sendAction("stop"); }}
        // style-as-function → per-press feedback: dip opacity + scale so the
        // tap is felt instantly. When stopping, tint the pill toward the accent
        // so the state change reads even before the card disappears.
        style={({ pressed }) => ({
      ...
      >
        {stopping ? (
          <>
            <Spinner color={c.accent} size={11} />
            <Text style={{ fontSize: 13, lineHeight: 18, fontWeight: "600", color: c.accent }}>Stopping…</Text>
          </>
        ) : (
          <>
